...
const orderedListToText = line => {
    // line = line.trim();
    if (startsWithDigit(line) && (line.match(/./g) || []).length > 0 && !openCodeBlock) {
        parts = line.split(".");
...
const parserPipeline = pipe([codeHighlightToText, codeLineToText, headersToText, horizontalRuleToText, unorderedListToText, orderedListToText]);
